...
function SolveForFour(p) {
	...
	let
		...
		pp = LinesItersection(GetCoefficients(ru, lu), GetCoefficients(ld, rd));
	
	if (pp) {
		if (pp.x >= min([lu.x, ld.x, ru.x, rd.x]) && pp.x <= max([lu.x, ld.x, ru.x, rd.x])) {
			let temp = ru
			ru = rd
			rd = temp
		}
...
function LinesItersection(cof1, cof2) {
	if (cof1.K == cof2.K)
		return undefined

	let a = (cof2.B - cof1.B) / (cof1.K - cof2.K)

	return {x: a, y: cof1.K * a + cof1.B}
}
